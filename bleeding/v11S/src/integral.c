...
ML_API double ml_integral_traditional(double a, double b, double exponent, double additive, double d) {
    ...
    for (int step = 0; step < max_steps; step++) {
        ...
        if (ML_UNLIKELY(next_x == x)) {
            return 0.0 / 0.0; // Signal failure due to precision stall
        }
        x = next_x;
    }
...
ML_API double ml_gamma_new(double x) {
    if (ml_isnan(x)) return 0.0 / 0.0;
    if (x <= 0.0) return 0.0 / 0.0; // Undefined for 0 and negative integers
    if (x > 171.0) return 1.0 / 0.0; // 172! overflows double (approx 1.24e309)
