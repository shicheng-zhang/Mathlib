...
ML_API double ml_sin_scientific(double x) { return ml_minimax_sin(x); }

ML_API double ml_cos_scientific(double x) { return ml_minimax_cos(x); }

ML_API double ml_sin_graphics(double x) { return ml_minimax_sin(x); }

ML_API double ml_cos_graphics(double x) { return ml_minimax_cos(x); }

ML_API double ml_sin_fixed(double x) {
...
ML_API double ml_cos_fixed(double x) {
    x = ml_fmod(x, 2.0 * 3.14159265358979323846);
    ml_q16_16_t f_in = (ml_q16_16_t)(x * 65536.0);
    ml_q16_16_t s, c;
    ml_cordic_sincos_fixed(f_in, &s, &c);
...
ML_API double ml_tan(double x) {
    double s = ml_sin(x);
    double c = ml_cos(x);
    if (c == 0.0) return (s == 0.0) ? 0.0/0.0 : ((s > 0) ? 1.0/0.0 : -1.0/0.0);
    return s / c;
}
...
ML_API double ml_acot(double x) { return (math_pi / 2.0) - ml_atan(x); }
