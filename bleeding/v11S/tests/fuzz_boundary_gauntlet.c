...
void test_tensor_hilbert() {
    ...
    char scratchpad[4096]; // Reduced from 1MB for embedded safety
    ...
    // Verify residual: Ax should approximately equal b
    double residual_err = 0.0;
    for(int i=0; i<n; i++) {
        double ax_i = 0.0;
        for(int j=0; j<n; j++) ax_i += A_data[i*n+j] * x_data[j];
        residual_err += ml_fabs(ax_i - b_data[i]);
    }
    CHECK(residual_err < 1e-6, "Hilbert residual Ax == b");
}
