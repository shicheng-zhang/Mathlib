...
int main() {
    ...
    CHECK_NEAR(mean(data1, 5), 3);
    CHECK_NEAR(variance(data1, 5), 2);
    CHECK_NEAR(stddev(data1, 5), sqrt(2));
    float data2[] = {2, 4, 4, 4, 5, 5, 7, 9};
    CHECK_NEAR(mean(data2, 8), 5);
    CHECK_NEAR(variance(data2, 8), 4);
    CHECK_NEAR(stddev(data2, 8), 2);
    float data3[] = {10, 10, 10};
    ...
    CHECK_NEAR(binomial_pmf(10, 5, 0.5), 0.246094);
    CHECK_NAN(binomial_pmf(5, -1, 0.5));
    CHECK_NAN(binomial_pmf(5, 6, 0.5));
    float z0 = normal_pdf(0, 0, 1);
    CHECK_NEAR(z0, 0.398942);
    CHECK_NEAR(normal_pdf(1, 0, 1), 0.241971);
    CHECK_NEAR(normal_pdf(0, 0, 2), 0.199471);
    CHECK_NEAR(normal_pdf(2, 2, 1), 0.398942);
    float lr_x[] = {1, 2, 3, 4, 5};
    float lr_y[] = {2, 4, 6, 8, 10};
    float m_slope, b_intercept;
