...
void test_exp_log_boundaries() {
    ...
    CHECK(isinf(logarithm(0.0)) && logarithm(0.0) < 0, "log(0) == -inf");
    CHECK(exponential(1000.0) == INFINITY, "exp(1000) == inf");
...
void test_fixed_point_saturation() {
    ...
    ml_q16_16_t max_val = INT32_MAX;
    ml_q16_16_t min_val = INT32_MIN;

    CHECK(ml_fixed_mul(max_val, max_val) == INT32_MAX, "MAX * MAX saturates to MAX");
    CHECK(ml_fixed_mul(min_val, max_val) == INT32_MIN, "MIN * MAX saturates to MIN");
    CHECK(ml_fixed_mul(min_val, min_val) == INT32_MAX, "MIN * MIN saturates to MAX");
...
void test_tensor_hilbert() {
    ...
    int has_nan = 0;
    for(int i=0; i<n; i++) if (isnan(x_data[i]) || isinf(x_data[i])) has_nan = 1;
